...
#' method returns objects of class \linkS4class{StQ} from the slot \code{Data}
#' of the input object \code{x}.
#'
...
#' QList[[2]]
#'
#' @include rawStQList-class.R getPeriods.rawStQList.R
#'
#' @import data.table
#'
#' @export
setMethod(
    ...
    function(x, i, j, ..., exact = TRUE){
        ...
        Data <- x@Data
        names(Data) <- getPeriods.rawStQList(x)
        output <- Data[[i]]
        return(output)
