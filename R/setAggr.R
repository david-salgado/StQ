...
setReplaceMethod(
    ...
    function(object, value){
        ...
        if (dim(object@Aggregates)[1] > 0){
            
            object@Aggregates <- new(Class = 'DDdt')
            setVNC(object) <- DDdtToVNC(value, 'Aggregates')
            
        }else{
            
            setVNC(object) <- DDdtToVNC(value, 'Aggregates') + getVNC(object)
        }
